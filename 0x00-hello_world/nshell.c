...
void execute_with_path(char *cmd, char *infile, char *outfile) {
    char *path = getenv("PATH");
    char *dir = strtok(path, ":");
    char progpath[MAX_CMD_LEN];
    char *argv[MAX_ARGS];
    int found = 0;

    while (dir != NULL && !found) {
        snprintf(progpath, sizeof(progpath), "%s/%s", dir, cmd);
        if (access(progpath, X_OK) == 0) {
            found = 1;
            argv[0] = progpath;
            parse(cmd, argv, &infile, &outfile);
            execute(argv, infile, outfile);
        }
        dir = strtok(NULL, ":");
    }
    ...
    while (1) {
        ...
        while (token != NULL) {
            ...
            while (dir != NULL) {
                ...
                snprintf(cmd, MAX_CMD_LEN, "%s/%s", dir, args[0]);
                if (stat(cmd, &sb) != -1) {
                    args[0] = cmd;
                    break;
                }
